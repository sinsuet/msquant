...
CONFIG = {
    ...
    "batch_size": 128,
    "epochs": 200,
    "lr": 0.0005,        # Transformer 建议使用较低学习率
    "d_model": 64,       # Transformer 特有参数
    "nhead": 4,          # Transformer 特有参数
    "num_layers": 2,     # Transformer 特有参数
...
class ExplainableTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 3. Pooling: 取时间维度的平均作为上下文 Context
        context = out.mean(dim=1)
        
        # 4. Generate Weights
        weights = self.weight_generator(context)
        
        # 5. Weighted Prediction (使用当前时刻的因子值进行加权)
        current_factors = x[:, -1, :]
...
def run_transformer_strategy():
    ...
    if not os.path.exists(CONFIG['report_path']):
        print("因子报告不存在，请先运行 batch_processor.py")
        return
    ...
    # --- 训练 ---
    model = ExplainableTransformer(len(feature_cols), CONFIG['d_model'], CONFIG['nhead'], CONFIG['num_layers']).to(CONFIG['device'])
    ...
    plt.plot(unique_dates, curve[1:], label=f'{CONFIG["model_name"]} Strategy', color='purple')
